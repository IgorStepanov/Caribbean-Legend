...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
	...
	{
		case "quests":
			dialog.text = NPCStringReactionRepeat(RandPhraseSimple("Qual tipo di domande?","Cosa vuoi, "+GetAddress_Form(NPChar)+"?"),"Hai già provato a farmi una domanda "+GetAddress_Form(NPChar)+"...","Hai parlato di una certa questione per la terza volta oggi...","Guarda, se non hai nulla da dirmi riguardo alle questioni del porto, allora non disturbarmi con le tue domande.","block",1,npchar,Dialog.CurrentNode);
